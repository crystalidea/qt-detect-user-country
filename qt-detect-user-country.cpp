...
namespace LocalizationHelpers
{
	...
	QLocale::Country detectUserCountry()
	{
		...
		HKEY hKey = NULL;
		::RegOpenKeyEx(HKEY_CURRENT_USER, L"Control Panel\\International\\Geo", 0, KEY_READ, &hKey);
		...
		if (hKey)
		{
			...
			if (::RegQueryValueEx(hKey, L"Nation", 0, NULL, (LPBYTE)szCountry, &dwDataSize) == ERROR_SUCCESS)
			{
				int nLocation = _wtoi(szCountry);
#define match_qt_country(x, y) if (nLocation == x) { country = y; break; }
				...
				while (true)
				{
					...
					//match_qt_country(304, Ashmore and Cartier Islands); // 155.4 km2, uninhabited external territory of Australia
					...
					//match_qt_country(323, Guantanamo Bay); // 2015: president Obama stated that Gitmo "is not who we are" and that "It's time to close Gitmo".
					match_qt_country(324, QLocale::Guernsey);
					//match_qt_country(325, Heard Island and McDonald Islands); // 372 km2, Australian external territory
		...
		QString countryCodeStr = StringHelpers::Mac::CFStringtoQString(countryCode);
